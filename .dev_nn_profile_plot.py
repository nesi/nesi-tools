...
import matplotlib
matplotlib.use('agg')
from matplotlib import pyplot as plt, colors, gridspec, ticker
import sys, time, datetime, numpy, math, re
...
try:
    (time_scale, time_unit) = pick_time_scale(batch_task[-2, 'EpochTime'] - base_start_time)
except ValueError:
...
if n_steps > 1:
    ...
    if job_id is not None:
        try:
            ...
            step_name_map = dict((n, ((n+' '+v) if len(n)<4 else n)) for (n,v) in (line.split('.', 1)[-1].split(None, 1) for line in check_output([
                'sacct', '-n', '-j', job_id, '--format', 'jobid%20,jobname'], universal_newlines=True).strip().split('\n')))
...
(fig, (cpu_subplot, mem_subplot, io_rate_subplot, cumulative_io_subplot, *gpu_subplots)) = plt.subplots(
...
for (i_step, step_name) in enumerate(step_order):
    ...
    for (i_node, node) in enumerate(steps[step_name]['Nodes'].values()):
        for task in node.get('Tasks', {}).values():
            ...
            dx = numpy.diff(x)
            dx = numpy.concatenate([[numpy.median(dx)], dx])  # pad and avoid div by zero
            ...
            ok = numpy.logical_and(cpu>=0, cpu<=max_cpu*100)  # Max 128 CPUS per task
            cpu_subplot.plot(x[ok], cpu[ok]/100, color=c, label=series_name, drawstyle="steps")
            ...
            if 'GPUUtilization' in task.dtype.fields:
                ...
                gpu_subplot.plot(x[ok], gpu[ok]/100, color=c, label=series_name, drawstyle="steps")
            ...
            for (series_name, line_style, sign) in [('ReadMB', '-', 1), ('WriteMB', ':', -1)]:
...
for (line_style, sign, colour, x, dx, y) in io_rate_plots:
    b = y!=0
    io_rate_subplot.bar(x[b]-dx[b], y[b]*sign/io_rate_scale, dx[b], color=colour, linewidth=0)
...
for (line_style, sign, colour, x, dx, y) in cumulative_io_plots:
    cumulative_io_subplot.plot(x, y/cumulative_io_scale, color=colour, linestyle=line_style)
